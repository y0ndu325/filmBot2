...
type Handler struct {
	...
	pendingTitles map[int64]string
}
...
func New(bot *tgbotapi.BotAPI, service *service.MovieService, config *config.Config) *Handler {
	return &Handler{
		...
		pendingTitles: make(map[int64]string),
	}
}
...
func (h *Handler) handleRandom(chatID int64) error {
	...
	if movie.PosterPath != "" {
		posterURl := h.service.TMDB().GetImageURL(movie.PosterPath)
	...
	msg := tgbotapi.NewMessage(chatID, fmt.Sprintf("Будем смотреть это!: %s", movie.Title, movie.Overview))
	_, err = h.bot.Send(msg)
	return err
}
...
func (h *Handler) handleDefault(chatID int64, text string) error {
	 if state, exists := h.states[chatID]; exists && state == "wait_year" {
        title := h.pendingTitles[chatID]
        year := strings.TrimSpace(text)
        err := h.service.AddMovieWithYear(title, year)
        delete(h.pendingTitles, chatID)
        delete(h.states, chatID)
        if err != nil {
            msg := tgbotapi.NewMessage(chatID, fmt.Sprintf("Ошибка при добавлении фильма: %v", err))
            _, _ = h.bot.Send(msg)
            return err
        }
        msg := tgbotapi.NewMessage(chatID, fmt.Sprintf("Фильм '%s' сохранен.", title))
        _, err = h.bot.Send(msg)
        return err
    }
    ...
    h.pendingTitles[chatID] = text
    h.states[chatID] = "wait_year"
    msg := tgbotapi.NewMessage(chatID, "Введите год фильма:")
    _, err := h.bot.Send(msg)
    return err
}